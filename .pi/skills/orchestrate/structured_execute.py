...
async def _review_code_runner_output(task: TaskRuntime, session_dir: Path) -> None:
    """T2 gate: invoke /review-code on code-runner's changes (best-effort).

    Generates a review request from the hunk.md diff, sends to review-code,
    and stores the review output. Non-fatal — code-runner result stands if
    review-code is unavailable or fails.
    """
    review_code = SKILLS_DIR / "review-code" / "run.sh"
    if not review_code.exists():
        return

    # Build review request from hunk.md (has the diff + trajectory)
    ...
    try:
        ...
        logger.info("T2 review completed for {}", task.task_id)
