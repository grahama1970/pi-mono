...
export function validateExecGraphPlan(graph: ExecGraph): PlanValidationResult {
  ...
  if (graph.self_improvement_iterations !== undefined && (!Number.isInteger(graph.self_improvement_iterations) || graph.self_improvement_iterations < 1)) {
    issues.push({ severity: "blocking", code: "invalid_self_improvement_iterations", message: "self_improvement_iterations must be a positive integer." });
  }
  ...
  for (const id of duplicateIds) {
    issues.push({ severity: "blocking", code: "duplicate_node_id", message: `Duplicate node id: ${id}`, node_id: id });
  }

  for (const node of graph.nodes) {
    if (!node.type.trim()) {
      issues.push({ severity: "blocking", code: "missing_node_type", message: "Node is missing type.", node_id: node.id });
    }
    if (!node.node_goal.trim()) {
      issues.push({ severity: "blocking", code: "missing_node_goal", message: "Node is missing goal.", node_id: node.id });
    }
    ...
    for (const dependency of dependencies) {
      if (dependency === node.id) {
        issues.push({ severity: "blocking", code: "self_dependency", message: "Node cannot depend on itself.", node_id: node.id });
      }
      if (seenDependencies.has(dependency)) {
        issues.push({ severity: "blocking", code: "duplicate_dependency", message: `Duplicate dependency: ${dependency}`, node_id: node.id });
      }
      seenDependencies.add(dependency);
      if (!ids.has(dependency)) {
        issues.push({ severity: "blocking", code: "missing_dependency", message: `Dependency does not exist: ${dependency}`, node_id: node.id });
      }
    }

    const promptLike = /prompt|llm|model|scillm|claude|openai|gemini|review-code/i.test(`${node.type} ${node.protocol_role ?? ""} ${node.node_goal}`);
    const reviewCodeLike = /review-code|review_code/i.test(`${node.type} ${node.protocol_role ?? ""} ${node.node_goal}`);
    if (promptLike && !node.prompt && !node.messages?.length && !node.review_scopes?.some((scope) => scope.prompt?.trim())) {
      issues.push({ severity: "warning", code: "missing_prompt_contract", message: "Prompt-like node has no prompt or messages.", node_id: node.id });
    }
    if (reviewCodeLike && !node.review_scopes?.length) {
      issues.push({ severity: "warning", code: "missing_review_scopes", message: "review-code node has no scoped review contracts.", node_id: node.id });
    }
    if (node.review_scopes?.length) {
      ...
      for (const scope of node.review_scopes) {
        ...
        if (!scope.agent?.trim()) {
          issues.push({ severity: "blocking", code: "missing_review_agent", message: "Enabled review fanout row is missing an agent.", node_id: node.id });
        }
        if (!contractName) {
          issues.push({ severity: "blocking", code: "missing_review_contract", message: "Enabled review fanout row is missing a contract.", node_id: node.id });
          continue;
        }
        if (!scope.model?.trim()) {
          issues.push({ severity: "blocking", code: "missing_review_model", message: `Review contract ${contractName} is missing a model.`, node_id: node.id });
        }
        if (isDeprecatedReviewModel(scope.model)) {
          issues.push({ severity: "blocking", code: "deprecated_review_model", message: `Review contract ${contractName} uses deprecated review model ${scope.model?.trim()}; choose a provider-family or exact model.`, node_id: node.id });
        }
        if (!scope.proof_level?.trim()) {
          issues.push({ severity: "blocking", code: "missing_review_proof_level", message: `Review contract ${contractName} is missing a proof level.`, node_id: node.id });
        }
        if (!scope.best_practice_skills?.length) {
          issues.push({ severity: "blocking", code: "missing_review_best_practices", message: `Review contract ${contractName} is missing best-practices skill inputs.`, node_id: node.id });
        } else {
          for (const skill of scope.best_practice_skills) {
            if (!String(skill ?? "").trim() || !String(skill).startsWith("best-practices-")) {
              issues.push({ severity: "blocking", code: "invalid_review_best_practices", message: `Review contract ${contractName} must use best-practices-* skill names.`, node_id: node.id });
            }
          }
        }
        if (seenContracts.has(contractName)) {
          issues.push({ severity: "blocking", code: "duplicate_review_contract", message: `Duplicate review contract: ${contractName}`, node_id: node.id });
        }
  ...
  for (const cycle of findCycles(graph)) {
    issues.push({ severity: "blocking", code: "cycle", message: `Cycle detected: ${cycle.join(" -> ")}`, node_id: cycle[0] });
  }
...
function changedFilePaths(options?: RuntimeReadinessOptions): string[] {
  return (options?.changed_files ?? []).flatMap((entry) => {
    ...
    if (entry && typeof entry === "object" && typeof entry.path === "string" && entry.path.trim()) return [entry.path];
    return [];
  });
}
...
export function analyzeExecGraphRuntimeReadiness(graph: ExecGraph, options: RuntimeReadinessOptions = {}): RuntimeReadinessReport {
  ...
  return {
    ...
    summary: {
      ...
      missing_fields_by_node: Object.fromEntries(nodes.filter((node) => node.missing_fields.length).map((node) => [node.node_id, node.missing_fields])),
    },
    nodes,
  };
}

function analyzeNodeRuntimeReadiness(node: ExecGraphNode, options: RuntimeReadinessOptions): RuntimeReadinessNodeReport {
  ...
  if (RUNTIME_NODE_TYPES.has(node.type)) {
    ...
    if (node.review_scopes?.length) {
      ...
      node.review_scopes.forEach((scope, index) => missing_fields.push(...reviewScopeMissingFields(scope, `review_scopes[${index}]`)));
    } else {
    ...
    if (!hasNodeValue(node, "phase", "phase_id")) inferred_fields.push({ field: "phase", source: "CLI --phase", value: options.phase_id ?? "" });
    ...
    if (hasNodeValue(node, "acceptance_evidence", "evidence_artifacts", "expected_artifacts")) present_fields.push("acceptance_evidence");
...
function validateDomainLimits(limits: ReviewDomainLimits | undefined, field: string, allowZero: boolean, issues: PlanValidationIssue[]) {
  ...
  for (const key of ["review_code", "review_design", "review_prompt"]) {
    ...
    if (!Number.isInteger(limit) || Number(limit) < minimum) {
      issues.push({ severity: "blocking", code: `invalid_${field}_${key}`, message: `${field}.${key} must be an integer >= ${minimum}.` });
    }
  }
}
...
export function applyPlanPatch(graph: ExecGraph, patch: PlanPatch): PlanPatchResult {
  ...
  if (patch.op !== "add_node" && !node) {
    return { graph, applied: false, issue: { severity: "blocking", code: "node_not_found", message: `Node not found: ${patch.node_id}`, node_id: patch.node_id } };
  }
  ...
  if (patch.op === "add_node") {
    if (next.nodes.some((candidate) => candidate.id === patch.node.id)) {
      return { graph, applied: false, issue: { severity: "blocking", code: "duplicate_node_id", message: `Duplicate node id: ${patch.node.id}`, node_id: patch.node.id } };
    }
  ...
  if (patch.op === "add_dependency") {
    if (!next.nodes.some((candidate) => candidate.id === patch.depends_on)) {
      return { graph, applied: false, issue: { severity: "blocking", code: "missing_dependency", message: `Dependency does not exist: ${patch.depends_on}`, node_id: patch.node_id } };
    }
  ...
  for (const node of draftGraph.nodes) {
    ...
    for (const field of EDITABLE_NODE_FIELDS) {
      if (!semanticEqual(base[field], node[field])) {
        diffs.push({ kind: "node_updated", node_id: node.id, field, label: `Updated ${node.id}.${field}`, before: base[field], after: node[field] });
      }
    }
    ...
    for (const dependency of draftDeps) {
      if (!baseDeps.has(dependency)) {
        diffs.push({ kind: "dependency_added", node_id: node.id, dependency, label: `Added dependency ${dependency} -> ${node.id}`, before: base.depends_on ?? [], after: node.depends_on ?? [] });
      }
    }
    for (const dependency of baseDeps) {
      if (!draftDeps.has(dependency)) {
        diffs.push({ kind: "dependency_removed", node_id: node.id, dependency, label: `Removed dependency ${dependency} -> ${node.id}`, before: base.depends_on ?? [], after: node.depends_on ?? [] });
      }
    }
  }
