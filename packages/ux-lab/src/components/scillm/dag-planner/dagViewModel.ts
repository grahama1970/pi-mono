...
export type AmendmentOperation =
  ...
  | { op: "supersede_node"; nodeId: string; priorRevisionId: string; newRevision: ExecGraphNodeDraft; rewirePolicy: "future_edges_only" | "explicit_edges"; rationale?: string }
...
export function buildDagViewModel(input: BuildDagViewModelInput): DagViewModel {
  ...
  const draft = input.diff?.length
    ? buildAmendmentDraft(input.graph, input.baseGraphHash, input.draftBaseGraphHash ?? input.baseGraphHash, input.diff, input.validation)
    : undefined;
...
function deriveEdges(nodes: ExecGraphNode[]): ExecGraphEdge[] {
  return nodes.flatMap((node) => (node.depends_on ?? []).map((source) => ({
    id: `${source}->${node.id}`,
    source,
    target: node.id,
  })));
...
function deriveReviewLanes(nodes: ExecGraphNode[]): ReviewLane[] {
  ...
  return laneSpecs.map((lane) => ({
    id: lane.id,
    label: lane.label,
    nodeIds: nodes.filter((node) => lane.match.test(`${node.id} ${node.type} ${node.protocol_role ?? ""} ${node.node_goal}`)).map((node) => node.id),
    source: "derived" as const,
  }));
}

function deriveRounds(nodes: ExecGraphNode[]): RoundView[] {
  const roundOneNodes = nodes
    .filter((node) => /round-1|round_1/i.test(node.id))
    .map((node) => node.id);
  return [{
    id: "round-1",
    label: "Round 1",
    nodeIds: roundOneNodes,
    materialized: roundOneNodes.length > 0,
    status: roundOneNodes.length > 0 ? "materialized" : "draft",
  }];
}

function deriveExecution(events: ExecEvent[], states: Record<string, ExecNodeState>, status?: ExecStatus): ExecutionView {
...
function buildAmendmentDraft(graph: ExecGraph, baseGraphHash: string, draftBaseGraphHash: string, diff: PlanDiffItem[], validation?: PlanValidationResult): AmendmentDraft {
  return {
...
function diffToOperation(diff: PlanDiffItem): AmendmentOperation {
  if (diff.kind === "dependency_added" && diff.dependency) return { op: "add_edge", edge: { source: diff.dependency, target: diff.node_id } };
  if (diff.kind === "dependency_removed" && diff.dependency) return { op: "remove_edge", edgeId: `${diff.dependency}->${diff.node_id}` };
  if (diff.kind === "node_added" && diff.after && typeof diff.after === "object") return { op: "add_node", node: diff.after as ExecGraphNodeDraft, draftOnly: true };
  ...
  if (diff.kind === "node_updated" && diff.field === "gate_policy") return { op: "update_gate", nodeId: diff.node_id, patch: diff.after as GatePolicyPatch };
  if (diff.kind === "node_updated" && diff.field === "retry_policy") return { op: "update_retry_policy", nodeId: diff.node_id, patch: diff.after as RetryPolicyPatch };
  if (diff.kind === "node_updated" && diff.field === "model") return { op: "update_model", nodeId: diff.node_id, value: diff.after };
  if (diff.kind === "node_updated" && diff.field === "persona_ref") return { op: "update_persona", nodeId: diff.node_id, value: diff.after };
  if (diff.kind === "node_updated" && diff.field === "prompt") return { op: "update_prompt", nodeId: diff.node_id, value: diff.after };
  if (diff.kind === "node_updated" && diff.field === "node_goal") return { op: "update_contract", nodeId: diff.node_id, value: diff.after };
