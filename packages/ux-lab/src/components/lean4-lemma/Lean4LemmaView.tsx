    ...
    <div style={{ display: "flex", height: "100%", background: EMBRY.bgDeep, color: EMBRY.white, fontFamily: "JetBrains Mono, monospace" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        ...
        <div style={{ padding: '6px 12px', borderBottom: `1px solid ${EMBRY.border}`, display: 'flex', alignItems: 'center', gap: 10, fontSize: 10 }}>
          <button data-qid="lean4:whatif:toggle" data-qs-action="TOGGLE_WHATIF" title="Toggle what-if analysis" onClick={() => { setWhatIfActive(!whatIfActive); if (whatIfActive) { setForcedSorry(new Set()); } }}
          ...
          {whatIfActive && <>
            ...
            <button data-qid="lean4:whatif:reset" data-qs-action="RESET_WHATIF" title="Reset what-if" onClick={() => setForcedSorry(new Set())} style={{ padding: '2px 8px', fontSize: 8, cursor: 'pointer', border: `1px solid ${EMBRY.border}`, background: 'transparent', color: EMBRY.dim, borderRadius: 2 }}>Reset</button>
            <button data-qid="lean4:impact:report" data-qs-action="SHOW_IMPACT_REPORT" title="Show impact report" onClick={() => setDetailTab('cascade')} style={{ padding: '2px 8px', fontSize: 8, cursor: 'pointer', border: `1px solid #FF9800`, background: '#FF980015', color: '#FF9800', borderRadius: 2 }}>Impact Report</button>
          </>}
          ...
          <div style={{ flex: 1, overflow: "auto" }}>
              ...
              <div style={{ padding: 16 }}>
                ...
                {forcedSorry.size === 0
                  ...
                  : <>
                    ...
                    {/* Re-prove button */}
                    <button data-qid="lean4:reprove:all" data-qs-action="REPROVE_ALL" title="Re-prove sorry lemmas" onClick={() => {
            ...
            {detailTab === "chat" && (
              <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
                ...
                <form data-qid="lean4:chat:form" data-qs-action="SUBMIT_CHAT" title="Submit message" onSubmit={handleChatSubmit} style={{ padding: "6px 10px", borderTop: `1px solid ${EMBRY.border}`, display: "flex", gap: 6 }}>
                  <input data-qid="lean4:chat:input" data-qs-action="CHAT_INPUT" title="Ask about proofs" id="lean4-chat-input" value={chatInput} onChange={e => setChatInput(e.target.value)}
                  ...
                  <button data-qid="lean4:chat:submit" data-qs-action="SUBMIT_CHAT" title="Send message" type="submit" disabled={chatLoading} style={{ padding: "5px 12px", background: EMBRY.accent, border: "none", borderRadius: 4, color: "#000", fontSize: 10, fontWeight: 700, cursor: chatLoading ? "wait" : "pointer" }}>Ask</button>
                </form>
              </div>
            )}
          </div>
