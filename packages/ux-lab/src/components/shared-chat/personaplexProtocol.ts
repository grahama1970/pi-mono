...
export function extractGeneratedText(event: PersonaPlexServerEvent): string | undefined {
	...
	const payloadCandidates =
		payload && typeof payload === "object"
			...
			: [];
	const payloadText = payloadCandidates.find(
...
export function traceRowsFromEvents(events: PersonaPlexServerEvent[]): PersonaPlexTraceRow[] {
	...
	for (const event of events) {
		...
		if (event.type === "grounding_stage_complete" || event.status === "complete") {
			...
			if (
				stage.includes("answer") ||
				stage.includes("clarify") ||
				stage.includes("deflect") ||
				stage.includes("voice_output")
			) {
...
export async function sendPersonaPlexSpeechFinalBestEffort(
	...
	return await new Promise<PersonaPlexSendResult>((resolve) => {
		...
		socket.onmessage = (message) => {
			...
			if (
				event.type === "personaplex_turn_complete" ||
				event.type === "assistant_response" ||
				event.type === "answer" ||
				(event.status === "complete" && event.stage === "voice_output") ||
				audioUrl
			) {
				globalThis.clearTimeout(timer);
				finish();
		...
		socket.onclose = () => {
			...
			finish({
				error:
					generatedText || audioUrl || sent ? undefined : "PersonaPlex WS closed before speech_final was accepted",
			});
		};
	});
}
