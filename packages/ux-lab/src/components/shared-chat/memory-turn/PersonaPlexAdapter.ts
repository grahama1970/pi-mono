...
export class PersonaPlexAdapter implements MemoryTurnAdapter {
	...
	private async *consumePersonaEvents(
		...
		const finalText =
			content.trim() ||
			extractContentFromUnknown(finalPayload) ||
			"PersonaPlex completed without a renderable answer.";
		const message = makeFinalMessage({
			branch: "personaplex",
			content: finalText,
			reasoningSteps: streamingStepsToThinkingTrace(emittedSteps),
			metadata: {
				...
				finalPayload,
				deprecatedLayout:
					"PersonaPlexChatWell grid/CSS is gallery-only; production renders through ComplianceChatWell.",
			},
		});
		yield makeFinalStep(message, "personaplex");
		return message;
...
function personaStepId(value: string | undefined): StreamingStepId | undefined {
	...
	if (normalized.includes("memory") || normalized.includes("recall") || normalized.includes("retrieve"))
		return "persona-recall";
	if (normalized.includes("answer") || normalized.includes("respond") || normalized.includes("compose"))
...
function normalizeStatus(value: unknown): "pending" | "running" | "completed" | "failed" | "skipped" {
	...
	if (text === "pending" || text === "running" || text === "completed" || text === "failed" || text === "skipped")
		return text;
...
function isTerminalPersonaEvent(event: unknown): boolean {
	...
	const type = stringValue(
		(parsed as UnknownRecord).type ?? (parsed as UnknownRecord).event ?? (parsed as UnknownRecord).kind,
	)?.toLowerCase();
