...
export class SpartaComplianceAdapter implements MemoryTurnAdapter {
	...
	private async *runComplianceCascade(
		...
		const entities = await this.postJson<UnknownRecord>(
		...
		const trainingRecall = await this.postJson<RecallResult>(
			"/api/memory/recall",
			{
				q: query,
				query,
		...
		yield pushStep(emittedSteps, {
			...
			status: "completed",
			detail:
				recallConfidence === undefined ? "Recall completed" : `Recall confidence ${recallConfidence.toFixed(2)}`,
			data: trainingRecall,
		});
		...
		if (gateState.needsClarification) {
			...
			const clarification = await this.postJson<ClarifyResult>(
				"/api/memory/clarify",
				{
					q: query,
					query,
		...
		const intent = await this.postJson<UnknownRecord>(
			"/api/memory/intent",
			{
				q: query,
				query,
		...
		if (isVisualPresentationIntent(intent)) {
			yield pushStep(emittedSteps, {
				id: "answering",
				branch,
				status: "completed",
				detail: visualPresentationSummary(intent),
			});
			const message = makeFinalMessage({
				branch,
				content: visualPresentationContent(query),
				reasoningSteps: streamingStepsToThinkingTrace(emittedSteps),
				metadata: {
					entities,
					trainingRecall,
					gates: gateState,
					intent,
					toolCalls: recordValue(intent, "tool_calls"),
					routedBy: "memory.intent",
				},
			});
			yield makeFinalStep(message, branch);
			return message;
		...
		const memoryResults = await this.postJson<RecallResult>(
			"/api/memory/recall",
			{
				q: query,
				query,
		...
		if (!content) {
			fallbackUsed = true;
			const fallback = await this.postJson<UnknownRecord>(
				"/api/memory/answer",
				{
					q: query,
					query,
			...
			content =
				extractContentFromUnknown(fallback) ||
				"I could not find enough grounded SPARTA memory to answer this turn.";
		}
		yield pushStep(emittedSteps, {
			id: "answering",
			branch,
	...
	private async *runEvidenceCaseTurn(
		...
		let content = "";
		let receipt: unknown;
		for await (const event of this.postJsonStream(
			this.evidenceCaseEndpoint,
			{
				query: evidenceQuery,
				text: evidenceQuery,
				matrix_context: input.matrixContext ?? this.matrixContext,
	...
	private async *runAqlTurn(
		...
		const result = await this.postJson<RecallResult>(
			"/api/memory/recall",
			{
				q: aql || query,
				type: "aql",
	...
	private async *runUtilityTurn(query: string, emittedSteps: StreamingStep[]): MemoryTurnStream {
		...
		const message = makeFinalMessage({
			branch,
			content:
				answer ?? "I can only answer deterministic utility turns here. Try a SPARTA compliance question instead.",
			reasoningSteps: streamingStepsToThinkingTrace(emittedSteps),
...
function isVisualPresentationIntent(intent: UnknownRecord): boolean {
	const responseMode = recordValue(intent, "response_mode");
	const questionKind = recordValue(intent, "question_kind");
	const source = recordValue(intent, "classifier_source");
	return (
		responseMode === "image_and_markdown_table" ||
		questionKind === "image_plus_metrics_table" ||
		source === "deterministic_visual_presentation"
	);
}

function visualPresentationSummary(intent: UnknownRecord): string {
	const toolCalls = recordValue(intent, "tool_calls");
	if (Array.isArray(toolCalls)) {
		const names = toolCalls
			.map((call) => (call && typeof call === "object" ? recordValue(call as UnknownRecord, "skill") : undefined))
			.filter(Boolean)
			.map(String);
		if (names.length) return `Routed by $memory intent to ${names.map((name) => `$${name}`).join(" -> ")}`;
	}
	return "Routed by $memory intent to visual presentation";
}

function visualPresentationContent(query: string): string {
	if (isCatMetricsQuery(query)) {
		return [
			"## Cat Video Snapshot",
			"",
	...
	return [
		"I can route visual presentation requests through `$memory intent` and render the requested artifact format here.",
		"",
		"| Route | Status |",
		"| --- | --- |",
		"| `$memory intent` | selected visual presentation |",
		"| artifact rendering | ready |",
	].join("\n");
}

function isCatMetricsQuery(query: string): boolean {
	const normalized = query.toLowerCase();
	return normalized.includes("cat") && normalized.includes("video") && normalized.includes("table");
}
...
function isInconclusiveClarification(clarification: ClarifyResult): boolean {
	...
	return (
		...
		Array.isArray(clarification.clarifyOptions)
	);
}
...
function mapEvidenceEventToStep(
	event: unknown,
	emittedSteps: StreamingStep[],
): { step?: StreamingStep; content?: string; receipt?: unknown } {
