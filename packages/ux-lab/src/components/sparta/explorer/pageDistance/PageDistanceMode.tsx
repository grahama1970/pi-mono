...
export function PageDistanceModeSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  useRegisterAction(CHAT_DISTANCE_QID['10ft'], {
    app: 'sparta-explorer',
    action: 'PAGE_DISTANCE_10FT',
    label: 'Page glance (10ft)',
    description: 'Switch active Explorer page to 10ft wall glance',
  })
  useRegisterAction(CHAT_DISTANCE_QID['5ft'], {
    app: 'sparta-explorer',
    action: 'PAGE_DISTANCE_5FT',
    label: 'Page triage (5ft)',
    description: 'Switch active Explorer page to 5ft triage',
  })
  useRegisterAction(CHAT_DISTANCE_QID['lean-in'], {
    app: 'sparta-explorer',
    action: 'PAGE_DISTANCE_LEAN_IN',
    label: 'Page drilldown (lean-in)',
    description: 'Switch active Explorer page to lean-in drilldown',
  })

  if (!supportsPageDistance) return null

  const selectorModes = MODE_OPTIONS.map(({ mode: optionMode, label, hint }) => ({
    id: optionMode,
    label,
    hint,
