      ...
      <div style={{ padding: '12px 16px', borderBottom: `1px solid ${EMBRY.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 6 }}>
          {allFrameworks.map((fw) => {
            ...
            return <button key={fw} data-qid={`lemma:fw:${fw}`} data-qs-action="TOGGLE_FRAMEWORK_FILTER" title={`Toggle ${fw} framework filter`} data-qs-params={JSON.stringify({ framework: fw })} onClick={() => toggleFramework(fw)} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '3px 8px', borderRadius: 4, cursor: 'pointer', border: `1px solid ${isActive ? color : EMBRY.border}`, backgroundColor: isActive ? `${color}18` : 'transparent', opacity: isActive ? 1 : 0.4, transition: 'all 0.15s', minHeight: 44, minWidth: 44, justifyContent: 'center' }}><div style={glowDot(color, 6)} /><span style={{ fontSize: 9, color: isActive ? color : EMBRY.dim, fontWeight: 700 }}>{fw}</span></button>
          })}
          {activeFrameworks.size > 0 && <button data-qid="lemma:fw:reset" data-qs-action="RESET_FRAMEWORK_FILTER" title="Show all frameworks" onClick={() => setActiveFrameworks(new Set())} style={{ fontSize: 9, fontWeight: 700, padding: '3px 8px', borderRadius: 4, cursor: 'pointer', border: `1px solid ${EMBRY.border}`, backgroundColor: 'transparent', color: EMBRY.dim, minHeight: 44, minWidth: 44, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>ALL</button>}
        </div>
      </div>
