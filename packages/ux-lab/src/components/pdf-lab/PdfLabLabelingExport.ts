...
export interface BreadcrumbNode {
  ...
  source?: 'toc' | 'outline' | 'structure_tree' | 'section_hierarchy' | 'section_anchor' | 'reverse_scan' | 'preset' | 'agent_second_pass' | 'human'
  editable?: boolean
  page?: number
}
...
export function normalizeBreadcrumbNodes(value: unknown, fallback?: string[]): BreadcrumbNode[] | undefined {
  if (Array.isArray(value)) {
    ...
    value.forEach((rawNode, index) => {
      ...
      if (typeof record.parent_node_id === 'string' && record.parent_node_id.trim()) node.parent_node_id = record.parent_node_id.trim()
      if (typeof record.source === 'string' && record.source.trim()) node.source = record.source as BreadcrumbNode['source']
...
export function breadcrumbNodesFromRegion(region: Pick<ExportableRegion, 'breadcrumb' | 'breadcrumb_nodes'>): BreadcrumbNode[] {
...
export function breadcrumbMetadataChanged(
  ...
  return formatBreadcrumb(left.breadcrumb) !== formatBreadcrumb(right.breadcrumb) ||
...
export function breadcrumbNodeIdentityKey(node: BreadcrumbNode): string {
  return [
    ...
    node.label.trim(),
    node.node_id ?? '',
    Number.isFinite(node.page) ? String(node.page) : '',
  ].join('::')
}
