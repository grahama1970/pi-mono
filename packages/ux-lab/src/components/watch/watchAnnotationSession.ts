...
export function createWatchAnnotationSession(
	...
	const selectedActor =
		previous?.selectedActorName ||
		options.find((option) => characterKey(option.name) === characterKey(selectedName))?.actorName ||
		"";
...
export function setPlayheadSeconds(state: WatchAnnotationSessionState, seconds: number): WatchAnnotationSessionState {
...
function collectCandidateRecords(input: unknown, output: JsonRecord[] = [], seen = new Set<unknown>()): JsonRecord[] {
	...
	Object.entries(input).forEach(([key, value]) => {
		if (key === "bbox" || key === "normalized_bbox" || key === "box" || key === "basis" || key === "metadata") {
			return;
		}
...
function recordActor(record: JsonRecord): string {
	return firstString(record, ["actor_name", "actorName", "actor", "performer_name", "performerName"]);
}

function recordAssetUid(record: JsonRecord): string {
	return firstString(record, ["asset_uid", "assetUid", "movie_asset_uid", "movieAssetUid"]);
}
...
function parseKeyframeRecord(row: WatchAnnotationSceneRow, record: JsonRecord): WatchAnnotationKeyframe | null {
	...
	return {
		...
		detectorObservationRef: normalizeDetectorObservationRef(
			record.detector_observation_ref ?? record.detectorObservationRef,
		),
		raw: record,
	};
}
...
export function replaceCanonicalFromHydration(
	...
	const sortedTracks = Object.fromEntries(
		Object.entries(nextState.tracks).map(([key, track]) => [
			key,
			{
				...track,
...
export function findExactKeyframeForCharacter(
	...
	return (
		track.keyframes.find(
			(keyframe) => Math.abs(keyframe.timeSeconds - timeSeconds) <= WATCH_ANNOTATION_EXACT_EPSILON_SECONDS,
		) ?? null
	);
}
...
export function findExactSelectedKeyframe(state: WatchAnnotationSessionState): WatchAnnotationKeyframe | null {
	if (state.selectedOverlayId?.startsWith("exact:")) {
		for (const track of Object.values(state.tracks)) {
			const selected = track.keyframes.find(
				(keyframe) => `exact:${track.characterKey}:${keyframe.id}` === state.selectedOverlayId,
			);
...
function lastControlBetween(
	track: WatchAnnotationTrack,
	afterSeconds: number,
	atSeconds: number,
): WatchAnnotationControlMarker | null {
	return (
		[...track.controls]
			.reverse()
			.find(
				(control) =>
					control.timeSeconds > afterSeconds + WATCH_ANNOTATION_EXACT_EPSILON_SECONDS &&
					control.timeSeconds <= atSeconds + WATCH_ANNOTATION_EXACT_EPSILON_SECONDS,
			) ?? null
	);
}
...
export function upsertHumanKeyframe(
	...
	const exactIndex = track.keyframes.findIndex(
		(keyframe) => Math.abs(keyframe.timeSeconds - input.timeSeconds) <= WATCH_ANNOTATION_EXACT_EPSILON_SECONDS,
	);
	const keyframe: WatchAnnotationKeyframe = {
		id:
			input.id ||
			(exactIndex >= 0
				? track.keyframes[exactIndex].id
				: `local-keyframe-${Date.now().toString(36)}-${simpleHash(JSON.stringify(input))}`),
...
export function updateKeyframeBbox(
	...
	const tracks = Object.fromEntries(
		Object.entries(state.tracks).map(([key, track]) => {
...
export function appendTrackControlLocally(
	...
	return {
		...
		tracks: {
			...
			[track.characterKey]: {
				...track,
				controls: [...track.controls, marker].sort(
...
export function trackStats(state: WatchAnnotationSessionState): {
	exactKeyframes: number;
	controls: number;
	characters: number;
} {
