      ...
      <div style={{ padding: '6px 12px', borderBottom: `1px solid ${EMBRY.border}`, display: 'flex', gap: 12, fontSize: 9, alignItems: 'center', color: EMBRY.dim, flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          {Object.entries(NODE_TYPE_COLORS).map(([type, color]) => {
            ...
            return (
              <span key={type} id={`be-legend-${type}`} data-qid={`be:legend:filter:${type}`} data-qs-action="TOGGLE_TYPE_FILTER" title="Toggle node type filter" data-qs-params={JSON.stringify({ type })} onClick={() => onToggleTypeFilter?.(type)}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 4, alignItems: 'center' }}>
          {(['organic', 'hierarchical', 'stratified', 'clustered'] as const).map((mode) => (
            <button key={mode} data-qid={`be:layout:${mode}`} data-qs-action="SET_LAYOUT" title="Switch graph layout" data-qs-params={JSON.stringify({ mode })} onClick={() => setActiveLayout(mode)}
