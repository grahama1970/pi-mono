...
app.post('/api/projects/dream/voices/audition', async (req, res) => {
  try {
    ...
    const text = typeof req.body?.text === 'string' ? req.body.text.trim() : ''
    const refAudioPath = typeof req.body?.refAudioPath === 'string' ? req.body.refAudioPath : ''
    const tone = normalizeChatterboxTone(req.body?.tone)
    const requestedParalinguisticCue = typeof req.body?.paralinguisticCue === 'string'
      ? req.body.paralinguisticCue.trim().toLowerCase()
      : typeof req.body?.emotionTag === 'string'
        ? req.body.emotionTag.trim().toLowerCase()
        : ''
    const selectedParalinguisticCue = CHATTERBOX_ALLOWED_PARALINGUISTIC_CUES.has(requestedParalinguisticCue)
      ? requestedParalinguisticCue
      : ''
    const inlineParalinguisticCue = extractInlineParalinguisticCue(text)
    const paralinguisticCue = selectedParalinguisticCue || inlineParalinguisticCue
    const deliveryStage = normalizeChatterboxDeliveryStage(req.body?.deliveryStage, tone)
    const pauseBeforeMs = clampPauseMs(req.body?.pauseBeforeMs)
    const pauseAfterMs = clampPauseMs(req.body?.pauseAfterMs)
    if (!['embry', 'kai'].includes(character)) return res.status(400).json({ status: 'error', error: 'character must be embry or kai' })
    if (!text) return res.status(400).json({ status: 'error', error: 'text required' })
    if (text.length > 1200) return res.status(400).json({ status: 'error', error: 'text exceeds 1200 characters' })
    const cleanText = stripSpeechControls(text)
    if (!cleanText) return res.status(400).json({ status: 'error', error: 'text contains only unsupported speech controls' })
    const ttsRenderText = buildChatterboxRenderText(text, cleanText, selectedParalinguisticCue)
    const refReal = realpathSync(refAudioPath)
    if (!isAllowedDreamPath(refReal)) return res.status(403).json({ status: 'error', error: 'refAudioPath not allowed' })
    const chatterboxRefAudio = resolveChatterboxReferencePath(refReal)
    ...
    const upstream = await fetch(`${CHATTERBOX_AGENT_URL.replace(/\/+$/, '')}/synthesize`, {
      ...
      body: JSON.stringify({
        text: ttsRenderText,
        ref_audio: chatterboxRefAudio,
        label: `dream-${character}-${Date.now()}`,
        delivery_stage: deliveryStage,
        temperature: 0.7,
      }),
    })
    ...
    if (!upstream.ok || !payload?.ok) {
      return res.status(502).json({
        ...
        mocked: false,
        live: true,
        error: payload?.error || payload?.detail || `Chatterbox returned ${upstream.status}`,
        chatterbox: payload,
      })
    }
    ...
    const outDir = resolve(PERSONA_DREAM_OUTPUTS_DIR, 'dream-voice-auditions')
    await mkdir(outDir, { recursive: true })
    const textHash = createHash('sha256').update(text).digest('hex').slice(0, 12)
    ...
    const receipt = {
      schema: 'persona_dream.voice_audition_receipt.v1',
      created_at: new Date().toISOString(),
      mocked: false,
      live: true,
      character,
      answer_text: cleanText,
      answer_text_sha256: createHash('sha256').update(cleanText).digest('hex'),
      tts_render_text: ttsRenderText,
      tts_render_text_sha256: createHash('sha256').update(ttsRenderText).digest('hex'),
      tone,
      delivery_stage: deliveryStage,
      paralinguistic_cue: paralinguisticCue,
      chatterbox_tags: paralinguisticCue ? [paralinguisticCue] : [],
      selected_paralinguistic_cue: selectedParalinguisticCue,
      inline_paralinguistic_cue: inlineParalinguisticCue,
      pause_before_ms: pauseBeforeMs,
      pause_after_ms: pauseAfterMs,
      ref_audio: refReal,
      chatterbox_ref_audio: chatterboxRefAudio,
      chatterbox_agent_url: CHATTERBOX_AGENT_URL,
    ...
    res.json({
      ...
      character,
      audioPath: outPath,
      audioUrl: `/api/projects/dream/asset?path=${encodeURIComponent(outPath)}`,
      receiptPath,
      receiptUrl: `/api/projects/dream/asset?path=${encodeURIComponent(receiptPath)}`,
      durationSeconds: payload.duration_seconds,
      ttsRenderText,
      tone,
      deliveryStage,
      paralinguisticCue,
      chatterboxTags: paralinguisticCue ? [paralinguisticCue] : [],
      selectedParalinguisticCue,
      inlineParalinguisticCue,
      pauseBeforeMs,
      pauseAfterMs,
    })
  } catch (err) {
    res.status(503).json({
      ...
      live: true,
      error: err instanceof Error ? err.message : String(err),
      hint: `Ensure Chatterbox agent server is reachable at ${CHATTERBOX_AGENT_URL}`,
    })
  }
})
...
app.post('/api/projects/dream/brave-search', async (req, res) => {
  try {
    ...
    if (!apiKey) return res.status(502).json({ error: 'brave_search_api_key_not_configured' })
    const url = new URL('https://api.search.brave.com/res/v1/web/search')
    url.searchParams.set('q', query)
    url.searchParams.set('count', '5')
    const proxyReq = httpsRequest(url.toString(), {
      method: 'GET',
      headers: {
        'Accept': 'application/json',
      ...
      proxyRes.on('end', () => {
        ...
        try {
          ...
          const results = (parsed.web?.results ?? []).map((r: Record<string, string>) => ({
            title: r.title || '',
            url: r.url || '',
            snippet: r.description || '',
          }))
          res.json({ status: 'ok', query, results, total: parsed.web?.total ?? results.length })
        } catch {
          res.status(502).json({ error: 'brave_search_parse_failed', body: body.slice(0, 500) })
        }
      })
    })
    proxyReq.on('error', (err) => res.status(502).json({ error: 'brave_search_failed', detail: err.message }))
    proxyReq.setTimeout(10000, () => { proxyReq.destroy(); res.status(504).json({ error: 'brave_search_timeout' }) })
    proxyReq.end()
  } catch (err) {
