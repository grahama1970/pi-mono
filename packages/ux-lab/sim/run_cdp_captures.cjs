...
const GROUPS = {
  ...
  'scene-management':      [...PRE, {a:'eval',s:`(()=>{const inp=document.querySelector('input[placeholder*="Name"]');if(inp){const s=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,'value').set;s.call(inp,'analysis-v1');inp.dispatchEvent(new Event('input',{bubbles:true}));const btn=document.getElementById('be-scene-save');if(btn)btn.click();return 'saved scene'}return 'no save input'})()`},{a:'wait',ms:500},{a:'ss',n:'03-scene-saved'}],
