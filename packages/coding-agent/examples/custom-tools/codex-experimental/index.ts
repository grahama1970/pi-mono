...
const codexSchema = Type.Object({
	prompt: Type.String({
		description: "Task for Codex to execute. Be specific about what you want done.",
	}),
	model: Type.Optional(
		Type.String({
			description: "Model override (e.g., 'o3', 'o4-mini'). Uses Codex default if not specified.",
		}),
	),
	sandbox: Type.Optional(
		StringEnum(["read-only", "workspace-write"] as const, {
			description: "Sandbox mode. Defaults to 'read-only'. Use 'workspace-write' to allow file modifications.",
		}),
	),
	workDir: Type.Optional(
		Type.String({
			description: "Working directory for Codex. Defaults to current directory.",
		}),
	),
});
...
const factory: CustomToolFactory = (pi) => {
	const tool: CustomAgentTool<typeof codexSchema, CodexDetails> = {
		...
		async execute(
			...
			params: CodexParams,
			signal?: AbortSignal,
			onUpdate?: (result: { content: Array<{ type: "text"; text: string }>; details: CodexDetails }) => void,
		) {
			...
			return new Promise((resolve, reject) => {
				const safeResolve = (value: { content: Array<{ type: "text"; text: string }>; details: CodexDetails }) => {
					if (!settled) {
						settled = true;
						resolve(value);
					}
				...
				if (signal?.aborted) {
					safeResolve({
						content: [{ type: "text", text: "Codex execution aborted before start." }],
						details: {
							events: [],
							exitCode: null,
							streaming: false,
				...
				child.on("close", (code) => {
					...
					if (wasAborted) {
						...
						resultText = lastMessage || "(Codex completed with no message output)";
					}
				...
				child.on("error", (err) => {
					...
					if (nodeErr.code === "ENOENT") {
						errorMessage = "Codex CLI not found. Install with: npm install -g @openai/codex";
		...
		renderCall(args: CodexParams, theme: ThemeInterface) {
			...
			const prompt = args.prompt.length > 80 ? `${args.prompt.slice(0, 77)}...` : args.prompt;
		...
		renderResult(
			...
			if (details?.streaming) {
				...
				const statusText = lastEvent ? formatEventForDisplay(lastEvent) : "Starting...";
				return new Text(
			...
			if (isError || details?.error) {
				...
				return new Text(theme.fg("error", "[error] Codex: ") + theme.fg("muted", errorMsg), 0, 0);
			}
			...
			if (options.expanded) {
				return new Text(
					theme.fg("success", "[ok] ") +
						theme.fg("muted", `Codex completed (exit ${exitCode}, ${details?.events.length || 0} events)`) +
						"\n\n" +
						message,
					0,
					0,
			...
			return new Text(theme.fg("success", "[ok] ") + theme.fg("muted", "Codex: ") + preview, 0, 0);
		},
	};
